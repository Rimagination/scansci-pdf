...
scansci-pdf 是一个 MCP 服务器，提供 23 个工具，覆盖学术论文的搜索、下载、引文导出、Zotero 推送和 WebVPN 机构代理管理。支持 13+ 数据源并行下载，100+ 中国高校 WebVPN。
...
**调用约定：** 下面的示例都是 MCP 工具调用，不是 shell 命令。批量类工具会通过 MCP progress 通知进度；不要手动传 `ctx`，它由 MCP 客户端自动注入且不应出现在用户参数里。
...
批量下载、BibTeX 导入和 `resolve_and_download` 会自动上报 MCP progress。向用户汇报时优先总结成功/失败数量、失败 DOI 和下一步建议，不要把每条进度日志逐字刷屏。
